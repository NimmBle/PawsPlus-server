...
public class AnimalTypeRepository(PawsPlusDbContext db)
    : DataRepository<PawsPlusDbContext, Animal>(db),
        IAnimalTypeDomainRepository
{
    public async Task<Animal> Find(int id,
        CancellationToken cancellationToken = default)
        => await this
            .All()
            .FirstOrDefaultAsync(at => at.Id == id, cancellationToken);

    public async Task<List<Animal>> FindAll(IEnumerable<int> ids,
        CancellationToken cancellationToken = default)
        => await this
            .All()
